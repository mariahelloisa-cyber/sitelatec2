...
// ── Transition helpers ───────────────────────────────────────────────────────
// Drive the formation animation from a Transition value: its `duration`
 ...
 */
function BaseParticleText(props) {
  const {
    text,
    colors,
    mode,
  ...
  useEffect(() => {
    ...
    const palette = Array.isArray(colors) && colors.length > 0 ? colors : ['#40ffaa', '#40aaff', '#ff40aa', '#aa40ff'];
  ...
  }, [mode, replay, position, text, colorsKey, particleSize, particleCount, mcEnabled, mcRadius, mcForce, fontSize, autoFit, transitionKey, isStatic]);
...
export default function ParticleText(props) {
  return <BaseParticleText {...PRESET_PROPS} {...props} />;
}
